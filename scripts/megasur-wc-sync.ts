...
interface MegaSurProduct {
    ...
    subfamilia: string;
    stock: string;
    pvd: string; // Precio de compra (formato español)
    pvp: string; // Precio de venta (formato español)
    imagenes: { url: string }[];
...
function calculatePriceWithMargin(cost: number, marginPercent: number): number {
    let price = round2(cost * (1 + marginPercent / 100));
    if (price < 99) {
        price = round2(price + 5);
    }
    return price;
}
...
async function runDifferentialSync() {
    ...
    // --- PROCESAR PRODUCTOS (CREAR Y ACTUALIZAR) ---
        ...
        try {
            ...
            const hasValidPrice = pvp !== null && pvp > 0;
            ...
            if (!hasValidPrice) draftReasons.push('precio inválido');
            ...
            if (hasValidPrice) {
                const cost = pvd !== null && pvd > 0 ? pvd : pvp;
                const regularPrice = calculatePriceWithMargin(cost, 20);
                const promoPrice = calculatePriceWithMargin(cost, 10);
                const isWeeklyPromo = isWeeklyBrandProduct(product, activeWeeklyBrands) && promoPrice < regularPrice;

                productData.regular_price = regularPrice.toFixed(2);
                productData.sale_price = isWeeklyPromo ? promoPrice.toFixed(2) : '';
                productData.meta_data.push(
                    { key: '_iv_base_margin_percent', value: '20' },
                    { key: '_iv_weekly_margin_percent', value: isWeeklyPromo ? '10' : '0' },
                    { key: '_iv_weekly_promo_active', value: isWeeklyPromo ? '1' : '0' },
                );
            }
            ...
            if (existingProducts.length > 0) {
                ...
                if (!hasValidPrice) {
                    // Si el feed no trae precio válido, conservamos el precio actual para no romper catálogo.
