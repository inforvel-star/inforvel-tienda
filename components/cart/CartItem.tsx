...
export function CartItem({ item, compact = false }: CartItemProps) {
  ...
  return (
    <motion.div
      ...
      className={`flex gap-4 ${compact ? 'p-2' : 'p-4'} bg-zinc-950 rounded-lg border border-zinc-800 hover:border-zinc-700 transition-all`}
    >
      <div className={`relative ${compact ? 'w-16 h-16' : 'w-24 h-24'} flex-shrink-0 bg-zinc-900 rounded-md overflow-hidden`}>
        <Image
          src={item.image}
          alt={item.name}
          fill
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-2">
            <Button
              variant="outline"
              size="icon"
              className={`${compact ? 'h-7 w-7' : 'h-8 w-8'} border-zinc-700 text-white hover:bg-zinc-800 hover:text-white`}
            ...
            <span className={`font-medium ${compact ? 'text-sm' : 'text-base'} min-w-[2rem] text-center text-white`}>
              {item.quantity}
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              variant="ghost"
              size="icon"
              className={`text-red-500 hover:text-red-400 hover:bg-red-500/10 ${compact ? 'h-7 w-7' : 'h-8 w-8'}`}
